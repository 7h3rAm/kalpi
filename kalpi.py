...
class kalpi:
  ...
  def __init__(self):
    ...
    self.blogtheme = ["default", "bright"]
    self.codehilitetheme = ["autumn", "borland", "bw", "colorful", "default", "emacs", "friendly", "fruity", "github", "manni", "monokai", "murphy", "native", "package", "pastie", "perldoc", "tango", "trac", "vim", "vs", "zenburn"]

    # 0:default (dark bg), 1:bright
    self.blogtheme = self.blogtheme[0]
    # 6:friendly, 15:perldoc
    self.codehilitetheme = self.codehilitetheme[15]
  ...
  def gen_posts(self):
    for f in self.files:
      ...
      self.write_file(url=f["url"], data=self.env.get_template("post.html").render(post=f, posts=self.files, mdbaseurl=self.mdbaseurl, related_posts=related_posts[:self.relatedpostscount], baseurl=self.baseurl, blogtheme=self.blogtheme, codehilitetheme=self.codehilitetheme, date=self.date))

  def gen_index(self):
    self.write_file("index%s" % self.urlextension, self.env.get_template("index.html").render(posts=self.files[:self.homepostscount], baseurl=self.baseurl, data=self.data, blogtheme=self.blogtheme, codehilitetheme=self.codehilitetheme, date=self.date))

  def gen_archive(self):
    self.write_file("archive%s" % self.urlextension, self.env.get_template("archive.html").render(posts=self.files, baseurl=self.baseurl, blogtheme=self.blogtheme, codehilitetheme=self.codehilitetheme, date=self.date))
  ...
  def gen_tags(self):
    ...
    self.write_file("tags%s" % self.urlextension, self.env.get_template("tags.html").render(tags=tagsinfo, baseurl=self.baseurl, blogtheme=self.blogtheme, codehilitetheme=self.codehilitetheme, date=self.date))

  def gen_research(self):
    self.write_file("research%s" % self.urlextension, self.env.get_template("research.html").render(posts=self.files, baseurl=self.baseurl, data=self.data, blogtheme=self.blogtheme, codehilitetheme=self.codehilitetheme, date=self.date))
  ...
  def gen_sitemap(self):
    self.write_sitemap("sitemap.xml", self.env.get_template("sitemap.html").render(posts=self.files, baseurl=self.baseurl, blogtheme=self.blogtheme, codehilitetheme=self.codehilitetheme, date=self.date))
  ...
  def gen_stats(self):
    ...
    self.write_file("stats%s" % self.urlextension, self.env.get_template("stats.html").render(stats=self.format("```json\n%s\n```" % (json.dumps(stats, indent=2, sort_keys=True))), posts=self.files, baseurl=self.baseurl, blogtheme=self.blogtheme, codehilitetheme=self.codehilitetheme, date=self.date))
